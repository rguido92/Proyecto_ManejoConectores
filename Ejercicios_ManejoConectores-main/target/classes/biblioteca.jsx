...
// Mount/unmount helpers for the browser (used by index.html toggles)
function mountBiblioteca() {
  const root = document.getElementById('react-root');
  if (!root) return;
  if (window.__biblioteca_root) return;
  const domRoot = ReactDOM.createRoot(root);
  domRoot.render(<App />);
  window.__biblioteca_root = domRoot;
}

function unmountBiblioteca() {
  if (window.__biblioteca_root) {
    try { window.__biblioteca_root.unmount(); } catch(e){ /* ignore */ }
    window.__biblioteca_root = null;
  }
}

window.mountBiblioteca = mountBiblioteca;
window.unmountBiblioteca = unmountBiblioteca;

// --- End inlined remainder ---
